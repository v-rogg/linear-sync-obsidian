import { App, Editor, FuzzySuggestModal, Modal, moment, Notice, Plugin, Setting } from "obsidian";
						...
						if (moment(file.stat.mtime).isAfter(moment(issue.updatedAt))) {
							...
							if (checkboxMatch) {
								if (checkboxMatch[1] === "x" && issue.state.type !== "completed") {
									...
									new Notice(`Synced issue ${issue.identifier} as completed`);
									...
									new Notice(`Synced issue ${issue.identifier} as started`);
									...
									new Notice(`Synced issue ${issue.identifier} as todo`);
									...
									new Notice(`Synced issue ${issue.identifier} as cancelled`);
								}
							}
						}
					}
					...
					if (idMatch) {
						...
						if (moment(file.stat.mtime).isBefore(moment(issue.updatedAt))) {
							if (issue.state?.type === "completed") {
								line = line.replace(/- \[.\] /, "- [x] ");
								new Notice(`Marked issue ${issue.identifier} as Done`);
							} else if (issue.state?.type === "started") {
								line = line.replace(/- \[.\] /, "- [/] ");
								new Notice(`Marked issue ${issue.identifier} as In Progress`);
								...
								line = line.replace(/- \[.\] /, "- [-] ");
								new Notice(`Marked issue ${issue.identifier} as Canceled`);
							}
						}
					}
...
class AddUndoneIssueLinkModal extends FuzzySuggestModal<EnrichedLinearIssue> {
	...
	getItems(): EnrichedLinearIssue[] {
		return this.issues
			.filter((issue: EnrichedLinearIssue) => {
...
class CreateIssueModal extends Modal {
	...
	onOpen() {
		...
		new Setting(contentEl).setName("Issue Title").addText((text) =>
			text.onChange((value) => {
				this.result.title = value;
			}),
		);

		new Setting(contentEl).setName("Team").addDropdown((drop) => {
